...
CDEXPrimaryGeneratorAction::CDEXPrimaryGeneratorAction( CDEXParameterList *list) : G4VUserPrimaryGeneratorAction(),
																												 fCDEXGPS(0)
{
	fPrimaryE = 0;
	fInitialE = 1 * keV;
...
void CDEXPrimaryGeneratorAction::GeneratePrimaries(G4Event *anEvent)
{
	...
	PList->SetSrcType(fSrcType);
	PList->SetPrimaryName(fCDEXGPS->GetParticleDefinition()->GetParticleName());
	G4ParticleTable *particleTable = G4ParticleTable::GetParticleTable();
	...
	else if (mode == "CDEXFiberBucketSetup" || mode == "CDEX300LF" || mode == "CDEX300LG")
	{
		if (fSrcType == "Bucket")
		{
			G4ThreeVector CentCoord;
