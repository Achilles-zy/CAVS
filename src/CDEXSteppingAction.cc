...
CDEXSteppingAction::CDEXSteppingAction(
	CDEXTrackingAction *track, CDEXEventAction *evt, CDEXRunAction *run)
	: CDEXTrack(track), CDEXEvent(evt), CDEXRun(run)
{
	//PMT PDE
...
{
	const CDEXDetectorConstruction *CDEXCons = static_cast<const CDEXDetectorConstruction *>(G4RunManager::GetRunManager()->GetUserDetectorConstruction());
	...
	if (CDEXCons->GetMode() == "CDEXFiberBucketSetup" ||CDEXCons->GetMode() == "CDEX300LF" || CDEXCons->GetMode() == "CDEX300LG")
	{
