...
CDEXRunActionMaster::CDEXRunActionMaster(CDEXPrimaryGeneratorAction *gen, CDEXDetectorConstruction *det) : SiPMEventCount(0),
																										   FiberPhotonCount(0),
																										   BulkEventCount(0),
																										   ROIEventCount(0),
																										   VetoEventCount(0),
...
{
	fRunMasterMessenger = new CDEXRunMasterMessenger(this);
...
CDEXRunActionMaster::~CDEXRunActionMaster()
{
	delete G4AnalysisManager::Instance();
	delete G4AccumulableManager::Instance();
}

G4Run *CDEXRunActionMaster::GenerateRun()
{
	return new Run;
}
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

void CDEXRunActionMaster::BeginOfRunAction(const G4Run *aRun)
{
	auto analysisManager = G4AnalysisManager::Instance();
	auto accumulableManager = G4AccumulableManager::Instance();
...
void CDEXRunActionMaster::EndOfRunAction(const G4Run *aRun)
{
	const G4int rank = G4MPImanager::GetManager()->GetRank();
	G4String mergename = filename + "_" + fPrimaryGenerator->GetPrimaryName() + "_Merge";
	filename = filename + "_" + fPrimaryGenerator->GetPrimaryName() + "_Rank_" + std::to_string(rank);
	...
	if (fDetCons->GetMode() == "CDEX300")
	{
		...
		if (rank == 0)
		{
			analysisManager->OpenFile(mergename);
			analysisManager->Write();
			analysisManager->CloseFile();
		}
	}
...
void CDEXRunActionMaster::CDEX300Output(const G4Run *aRun)
{
	if (G4RunManager::GetRunManager()->GetRunManagerType() == 1)
	{
...
void CDEXRunActionMaster::CDEXFiberBucketSetupOutput(const G4Run *aRun)
{
	if (G4RunManager::GetRunManager()->GetRunManagerType() == 1)
	{
		G4cout << "End Running ..." << G4endl;
...
void CDEXRunActionMaster::CDEXArParametersTestOutput(const G4Run *aRun)
{
	if (G4RunManager::GetRunManager()->GetRunManagerType() == 1)
	{
		G4cout << "End Running ..." << G4endl;
...
void CDEXRunActionMaster::CDEXArExpOutput(const G4Run *aRun)
{
